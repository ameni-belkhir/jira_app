...
/**
   ...
   * - Les items `hideForAdmin` sont masqués pour l'Admin (menu Admin = 7 éléments).
   * - Les routes « Backlog » et « Tickets (Kanban) » sont construites
   *   dynamiquement à partir du projet / sprint sélectionné (`ProjectStateService`).
   */
  get navItems(): SidebarNavItem[] {
    ...
    ).map((item: NavItem) => this.toSidebarItem(item));
  ...
  /** Construit un item de menu, en substituant les routes dynamiques (Backlog / Tickets). */
  private toSidebarItem(item: NavItem): SidebarNavItem {
    let path: string | undefined;

    switch (item.key) {
      case 'backlog':
        path = this.projectState.getBacklogRoute();
        break;
      case 'kanban':
        path = this.projectState.kanbanRoute();
        break;
      default:
    ...
    this.subscription.add(
      this.router.events.subscribe(event => {
        if (event instanceof NavigationEnd) {
          ...
          // Force change detection to re-evaluate dynamic routes
          this.cdr.detectChanges();
        }
      })
    );

    // Recalcule les routes dynamiques (Backlog / Tickets) quand le contexte
    // projet / sprint change (ProjectStateService).
    this.subscription.add(
      this.projectState.changes$.subscribe(() => {
        this.cdr.detectChanges();
      })
    );

    // Initialise un projet / sprint par défaut (premier projet + premier sprint actif).
    this.projectState.ensureDefaults();
  ...
  /** Navigue vers le Kanban du sprint sélectionné (initialise le contexte si besoin). */
  private async navigateToTickets(): Promise<void> {
    await this.projectState.ensureDefaults();
    this.router.navigate([this.projectState.kanbanRoute()]);
    this.closeMobileSidebar();
  }

  /** Navigue vers le Backlog du projet sélectionné (initialise le contexte si besoin). */
  private async navigateToBacklog(): Promise<void> {
    await this.projectState.ensureDefaults();
    this.router.navigate([this.projectState.getBacklogRoute()]);
    this.closeMobileSidebar();
  }
