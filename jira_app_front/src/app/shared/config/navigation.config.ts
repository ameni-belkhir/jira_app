/**
 * Configuration centralisée du menu latéral (Sidebar) et des guards.
 *
 * Source unique de vérité : chaque page, sa route, son icône et les rôles
 * autorisés par défaut. Le filtrage final est appliqué par les PERMISSIONS
 * RÉELLES de l'utilisateur renvoyées par le backend (`AuthService.hasPermission`),
 * ce qui garantit que le Sidebar et les guards utilisent la même source.
 */
...
export interface NavItem {
  ...
  adminOnly?: boolean;
  /** Caché du menu latéral pour le rôle Admin (menu Admin réduit à 7 éléments). */
...
const ICONS = {
  ...
  kanban: `<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M5 3h4a1 1 0 011 1v16a1 1 0 01-1 1H5a1 1 0 01-1-1V4a1 1 0 011-1zm1 2v14h2V5H6zM10 8h4a1 1 0 011 1v11a1 1 0 01-1 1h-4a1 1 0 01-1-1V9a1 1 0 011-1zm1 2v9h2v-9h-2zM15 5h4a1 1 0 011 1v14a1 1 0 01-1 1h-4a1 1 0 01-1-1V6a1 1 0 011-1zm1 2v12h2V7h-2z" fill="currentColor"/></svg>`,
 ...
 * Ordre Admin : Dashboard → Projets → Backlog → Tickets (Kanban) → Messagerie
 *               → Gestion des Rôles → Déconnexion.
 * - `backlog` est dynamique : sa route est construite avec l'ID du projet sélectionné.
 * - `kanban` (Tickets) est dynamique : sa route est construite avec le projet et
 *   le sprint sélectionnés (`ProjectStateService`).
 * - Les pages `hideForAdmin` (Profil, Projets Admin, Statistiques Admin) sont
 *   masquées du menu Admin, mais restent visibles pour les autres rôles.
 */
